...
object BasicZIOSpec extends ZIOSpecDefault{
  ...
  val userIdGen: Gen[Random, UserID] = intGen.map(UserID(_))
  val emailAddressGen = Gen.alphaNumericStringBounded(3, 5).map(str =>
    EmailAddress(s"${str}@mail.com"))

  val userGen: Gen[Random, User] = for{
    id <- userIdGen
    email <- emailAddressGen
  } yield User(id, email)
    ...
    suite("effect testing")(
      ...
      test("test console")(
        for{
          ...
          hasFirst(equalTo("Как тебя зовут\n")) && hasSize(equalTo(3)) )
      ),
...
//      test("updateCounter")(
//        assertZIO(zioDS.ref.updateCounter)(equalTo(3))
//      ) @@nonFlaky,
      test("updateCounterRef")(
